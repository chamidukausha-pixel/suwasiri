...
3. e-Rx is live on Firestore (GP Care → Vault E-Prescription). Telehealth notes (`consultation_notes`) sync to Call + Vault treatment notes + GP Care history. Vault **Sync Lanka GP Care** publishes vaccine history only. Other EMR charts still use the web JSON store
