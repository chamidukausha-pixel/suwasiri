...
3. e-Rx is live on Firestore (exam room → Vault E-Prescription; telehealth → Call E-Prescription). Telehealth and exam-room SOAP notes (`consultation_notes`) sync to Vault treatment notes. Immunisations, allergies, pathology/imaging, and medical certificates from the patient profile sync to the matching Suwasiri Vault / Profile sections. Vault **Sync Lanka GP Care** publishes vaccine history to the GP. Other EMR charts still use the web JSON store
4. Telehealth: in-app / in-browser WebRTC is live (GP room cam + patient camera, in-call messages). TURN if calls fail behind strict NAT
