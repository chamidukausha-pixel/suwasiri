...
16. **Unique Health ID (walk-in register)** — Reception types the card number (e.g. Chamidu `SW3C6F5B5A27`, Sakuni `SW6CF9340271`) on Patient Clinical Records and **Sync to Portal** (or Enter in the name filter if the text is an SW… ID). GP Care looks up that live Suwasiri `users` file plus Vault labs and vaccine history, **registers the patient at this clinic**, and shows their details — reception does not re-type demographics. Hashed dummy files (wrong name for the same ID) are removed and the barcode API no longer invents them. Another hospital must enter the ID and sync again. Reception **Delete** / **Block** needs a comment; Hospital Super Admin approves it. Sync does **not** open the doctor clinical profile for reception.
