...
16. **Unique Health ID** — Reception types the card number (e.g. Chamidu `SW3C6F5B5A27`, Sakuni `SW6CF9340271`) and **Sync to Portal**. GP Care looks up that live Suwasiri `users` file plus **Vault lab reports** and **vaccine history**. The Unique Health ID box shows name, age, gender, DOB, blood group, NIC, phone, email, address, allergies, emergency contact, labs, and vaccines. Reception then **Save to Patient Clinical Records** so the file is stored at **that clinic only**. Another hospital must enter the ID and save again. Dummy barcode generation is not used for a real Unique Health ID. Reception **Delete** / **Block** needs a comment; Hospital Super Admin approves it. Save does **not** open the patient clinical profile for reception.
