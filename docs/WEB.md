...
**Platform Console:** **Operations & Governance** — create a medical centre with a Sri Lankan **district**, then click a hospital name to add Doctor / Receptionist / Nurse / other staff, or **Remove (resigned)** so they leave Practice Manager, clinic booking lists, and the Suwasiri app. **Doctors require a specialty** (Cardiologist, Dermatologist, … matching the Suwasiri Doctors tab). After save they appear in Practice Manager, Security & RBAC, login, and on the Suwasiri app (search by doctor name, clinic name, and district) so patients can book available times; booked slots stay locked. Reception **Delete** / **Block** on Patient Clinical Records appears here as a **notification** (badge on Platform Console); Super Admin Approve / Reject.
...
17. **Clinic doctors** — Platform Console adds a medical centre (with district) and a doctor with specialty; Firestore `clinic_centers` / `clinic_doctors` lists them on Suwasiri **Doctors**. Patients search by clinician name (e.g. Priyantha Silva, CK Rathnayake), clinic name (e.g. CKR clinic, PrimeCare Medical Centre - Colombo Central), and all 25 districts. Searching a clinic name shows that centre even if a specialty category was selected. Practice Manager roster hours publish onto the same doctor so the app only offers those times; booked times from the app and GP Care stay locked. **Remove (resigned)** deletes the doctor from GP Care and sets `clinic_doctors.active: false` so they leave Suwasiri search and booking. Checkout offers **Pay at counter**, **Online Debit/Card**, and **Manual Bank Slip** (PDF or photo on the invoice). Reception **Cash Settle** → **Settled**; app pay → **Paid by Suwasiri App**. Age and gender from the Unique Health ID / booking show under the name.
