...
11. **Reception booking (live)** — Front Desk **Book** and **Recalls & Reminders** In person / Video open the Book scheduler. Confirm writes Firestore `appointments` with `consultMode` `clinic` or `video` for that Suwasiri patient. Home shows **blue** (in person) or **purple** (video). The GP Care month calendar lists the name on the booked date. The patient also gets an in-app notification.
