...
**Receipts & Invoices** and **Clinical Audit Trail** use the same month calendar. Click a date to show that day’s booked patients in a **scrollable** invoice table (like Appointments & Lobby Queue), or that day’s consult/lab activity (audit). Unpaid / not-settled rows are highlighted (amber, overdue red) and listed first. Reception **Cash Settle** marks the row **Settled** (clinic JSON invoice + Firestore appointment). If the patient paid in the Suwasiri app (online debit/card or manual bank slip), status is **Paid by Suwasiri App**. Click a PDF or image slip to open it. Age and gender show under the name on Lobby, Patient Clinical Records, Unique Health ID sync, and invoices.
...
**Platform Console:** **Operations & Governance** — create a medical centre with a Sri Lankan **district** and optional **logo** (change or remove later), then click a hospital name to add Doctor / Receptionist / Nurse / other staff with an optional **doctor photo**, or **Remove (resigned)** so they leave Practice Manager, clinic booking lists, and the Suwasiri app. **Doctors require a specialty** (Cardiologist, Dermatologist, … matching the Suwasiri Doctors tab). After save they appear in Practice Manager, Security & RBAC, login, and on the Suwasiri app (search by doctor name, clinic name, and district) so patients can book available times and see the photo/logo; booked slots stay locked. Reception **Delete** / **Block** on Patient Clinical Records appears here as a **notification** (badge on Platform Console); Super Admin Approve / Reject.
