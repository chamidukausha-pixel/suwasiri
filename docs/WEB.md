...
11. **Reception booking (live)** — Front Desk **Book Active Appointment**, Lobby **Book Appointment**, **Patient Portal** Book, and **Recalls & Reminders** In person / Video open the Book scheduler. Pick a doctor registered at that medical centre: **available** times are selectable; **booked** times (including Suwasiri App checkouts such as 04 Sep 2026 09:30 and 11:15) list the patient name under that doctor and cannot be double-booked. Confirm writes Firestore `appointments` with `consultMode` `clinic` or `video`. Home shows **blue** (in person) or **purple** (video). The GP Care month calendar lists the name on the booked date. The patient also gets an in-app notification. **Online Public Booking** is removed — patients book via Suwasiri or Patient Portal / reception.
