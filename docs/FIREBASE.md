...
| `vault` | `patientId`, `title`, `issuedBy`, `date`, `metrics`, `kind` (`lab`), `category` (`Pathology` / `Imaging`), `source` (`gp_care` when a GP files pathology or imaging; `lankalab` when LankaLab syncs a completed report) | **read: any signed-in** (patient Vault + GP Care Unique Health ID sync of previous lab reports); **create: signed-in or `source == lankalab`** (GP Care staff write reviewed lab **and imaging**; LankaLab non-critical **App** sync writes Vault → Lab reports) |
...
| `clinic_sample_dispatches` | One doc per Sample Dispatch bag (`id` = `SC-…`). `clinicName`, `driverName`, `driverPhone`, `vehicleNo`, `sampleCount`, `status` (`COLLECTED` / `DELIVERED`), `collectedAt`, `deliveredAt`, `labName`, `labAddress`, `issuedPersonName`, `issuedDate`, `source` (`gp_care`) | **read: public** (LankaLab portal has no login yet); **write: signed-in** GP Care staff. Shown on LankaLab Transit Logistics → Clinic Sample Collection Log |
| `clinic_critical_alerts` | One doc per LankaLab **Critical** flag (`id` = `ll-{specimenId}`). `patientId`, `patientName`, `testType`, `specimenId`, `source` (`lankalab`) | **read/create/update: public** (lab portal has no login yet); **delete: signed-in**. GP Care opens the matching patient file in red |
