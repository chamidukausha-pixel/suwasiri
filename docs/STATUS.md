...
- [x] GP Exam Room **Appointments**: during a consult the doctor books a follow-up on **their own** available/booked times (saved weekly roster hours, 15-minute slots). Reception sees the **same** grid (Lobby calendar, Book Active Appointment, and the read-only patient file). Confirm writes Firestore `appointments` for that Suwasiri patient (Home **blue** / **purple**) **and** the clinic store. **Save Weekly Rosters** writes `patient_store.json` and Firestore `clinic_doctors.rosterHours` so hours do not reset overnight.
...
- [x] Booked vs available times stay in sync under each doctor: Suwasiri Doctors booking, GP Care **Book Active Appointment**, exam-room **Appointments**, Lobby calendar, and reception’s read-only patient file. Slots follow Practice Manager weekly hours (not a generic 09:00–17:45 list). Catalog **Dr. Chamidu Rathnayake** (`d-chamidu-rathnayake`) and Platform Console **Dr. Chamidu Kaushal Rathnayake** (`d-chamidu-kaushal-rathnayake`) are the same clinician. Opening Platform Console no longer wipes saved `rosterHours`.
