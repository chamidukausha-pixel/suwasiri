...
- [x] Platform Console / Practice Manager: medical centre **logo** and doctor **photo** (add / change / remove) sync to Suwasiri booking. Practice Manager add/remove doctors appear or disappear on Staff & Practitioners. MBS items mapped to medical certificate / repeat prescription / review results publish to Firestore `clinic_fee_schedules` (including `global`). **Home** Medical certificate / Repeat prescription / Review results bookings show **only that private fee** (defaults LKR 1000 / 1500 / 1200 if not yet synced). Regular **Book appointment** still uses the doctor’s consult fee. Purple Auto-Select Mon–Fri removed from weekly rosters.
