...
export default function ResultDeliveryManager({ orders, onCompletedNavigate }: ResultDeliveryManagerProps) {
  ...
  useEffect(() => {
    if (!deliveryQueue.some((o) => o.id === selectedOrderId)) {
      setSelectedOrderId(deliveryQueue[0]?.id || '');
    }
  }, [deliveryQueue, selectedOrderId]);

  const selectedOrder = deliveryQueue.find((o) => o.id === selectedOrderId) || deliveryQueue[0];

  const filteredOrders = deliveryQueue.filter((o) => {
      ...
      <div className="bg-white border border-[#c1c7cf] rounded-xl p-5 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-600 mt-1 max-w-2xl leading-relaxed">
            One central portal to access, review, and handle all patient diagnostic data in real-time. Eliminates hardcopy or faxed delays with MDT quick sharing and cumulative reporting.
          </p>
        </div>
              ...
              {filteredOrders.length === 0 && (
                <div className="py-8 text-center text-slate-400 text-xs italic">
                  {deliveryQueue.length === 0
                    ? 'All results on this list are completed. They now appear under Operations Overview → Active Lab Orders.'
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {selectedOrder ? (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[11px] font-black uppercase tracking-wide inline-flex items-center gap-1.5 shadow-sm"
                    >
            ...
            <div className="p-12 text-center bg-white border border-[#c1c7cf] rounded-xl text-slate-500">
              {deliveryQueue.length === 0
                ? 'Completed results have moved to Operations Overview → Active Lab Orders.'
